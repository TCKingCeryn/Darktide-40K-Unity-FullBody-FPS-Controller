...
public class ConvertTexturesToJpg : EditorWindow
{
    private const string DUMMY_TEXTURE_PATH = "Assets/convert_dummyy_texturee.jpg";
    ...
    private readonly GUIContent optijpgPathContent = new GUIContent("Optijpg Path (Optional):", "If 'optijpg.exe' is selected, it will be used to reduce the image sizes even further (roughly 20%) but the process will take more time");
    private readonly GUIContent optijpgOptimizationContent = new GUIContent("Optijpg Optimization:", "Determines how many trials Optijpg will do to optimize the image sizes. As this value increases, computation time will increase exponentially");
    private readonly GUILayoutOption GL_WIDTH_25 = GUILayout.Width(25f);

    private string rootPath = "";
    private string textureExtensions = ".tga;.psd;.tiff;.tif;.bmp;.png";
    private string excludedDirectories = "";
    private bool keepOriginalFiles = false;
    private int maxTextureSize = 4096;
    private string optijpgPath = "";
    private int optijpgOptimization = 3;

    private Vector2 scrollPos;

    [MenuItem("Planet Maenad/Convert/Convert Textures to JPG")]
    private static void Init()
    {
        ConvertTexturesToJpg window = GetWindow<ConvertTexturesToJpg>();
        window.titleContent = new GUIContent("Convert to JPG");
    ...
    private void OnGUI()
    {
        ...
        optijpgPath = PathField(optijpgPathContent, optijpgPath, false, "Choose optijpg.exe path");
        if (!string.IsNullOrEmpty(optijpgPath))
        {
            EditorGUI.indentLevel++;
            optijpgOptimization = EditorGUILayout.IntSlider(optijpgOptimizationContent, optijpgOptimization, 2, 7);
            EditorGUI.indentLevel--;
        }

        EditorGUILayout.Space();

        // Convert Textures to jpg
        if (GUILayout.Button("Convert to jpg!"))
        {
            double startTime = EditorApplication.timeSinceStartup;

            List<string> convertedPaths = new List<string>(128);
            long originalTotalSize = 0L, convertedTotalSize = 0L, convertedTotalSizeOptijpg = 0L;

            try
            {
                rootPath = rootPath.Trim();
                excludedDirectories = excludedDirectories.Trim();
                textureExtensions = textureExtensions.ToLowerInvariant().Replace(".jpg", "").Trim();
                optijpgPath = optijpgPath.Trim();

                if (rootPath.Length == 0)
                    rootPath = Application.dataPath;

                if (optijpgPath.Length > 0 && !File.Exists(optijpgPath))
                    Debug.LogWarning("Optijpg doesn't exist at path: " + optijpgPath);
                ...
                for (int i = 0; i < paths.Length; i++)
                {
                    ...
                    string jpgFile = Path.ChangeExtension(paths[i], ".jpg");
                    string jpgMetaFile = jpgFile + ".meta";
                    ...
                    // Convert the Texture to jpg and save it
                    byte[] jpgBytes = AssetDatabase.LoadAssetAtPath<Texture2D>(DUMMY_TEXTURE_PATH).EncodeToJPG();
                    File.WriteAllBytes(jpgFile, jpgBytes);

                    originalTotalSize += new FileInfo(paths[i]).Length;
                    convertedTotalSize += new FileInfo(jpgFile).Length;

                    // Run Optijpg to optimize the jpg
                    if (optijpgPath.Length > 0 && File.Exists(optijpgPath))
                    {
                        try
                        {
                            Process.Start(new ProcessStartInfo(optijpgPath)
                            {
                                Arguments = string.Concat("-o ", optijpgOptimization.ToString(), " \"", jpgFile, "\""),
                                CreateNoWindow = true,
                        ...
                        convertedTotalSizeOptijpg += new FileInfo(jpgFile).Length;
                    }

                    // If .meta file exists, copy it to jpg image
                    if (File.Exists(originalMetaFile))
                    {
                        File.Copy(originalMetaFile, jpgMetaFile, true);

                        // Try changing original meta file's GUID to avoid collisions with jpg 
                        ...
                        if (isPSDImage)
                        {
                            string metaContents = File.ReadAllText(jpgMetaFile);
                            ...
                            if (modifiedMeta)
                                File.WriteAllText(jpgMetaFile, metaContents);
                        }
                    }
            ...
            finally
            {
                ...
                // Force Unity to import jpg images (otherwise we'd have to minimize Unity and then maximize it)
                AssetDatabase.Refresh();

                // Print information to Console
                StringBuilder sb = new StringBuilder(100 + convertedPaths.Count * 75);
                sb.Append("Converted ").Append(convertedPaths.Count).Append(" Texture(s) to jpg in ").Append((EditorApplication.timeSinceStartup - startTime).ToString("F2")).Append(" seconds (").
                    Append(EditorUtility.FormatBytes(originalTotalSize)).Append(" -> ").Append(EditorUtility.FormatBytes(convertedTotalSize));

                if (convertedTotalSizeOptijpg > 0L)
                    sb.Append(" -> ").Append(EditorUtility.FormatBytes(convertedTotalSizeOptijpg)).Append(" with Optijpg");
    ...
    private void CreateDummyTexture()
    {
        if (!File.Exists(DUMMY_TEXTURE_PATH))
        {
            File.WriteAllBytes(DUMMY_TEXTURE_PATH, new Texture2D(2, 2).EncodeToJPG());
            AssetDatabase.ImportAsset(DUMMY_TEXTURE_PATH, ImportAssetOptions.ForceUpdate);
        }
