...
namespace PlanetMaenad.FPS
{
    public class FPSBodyController : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            maxCamOriginal = PlayerArmsController.maxPitch;
        ...
        void CheckAnimator()
        {
            ...
            if (prone)
            {
                ...
                PlayerArmsController.maxPitch = 45;
            }
            else
            {
                _animator.SetBool("prone", false);
                PlayerArmsController.maxPitch = maxCamOriginal;
            }
        ...
        void StandardMove()
        {
            if (enabled)
            {
                ...
                //MainCamera Rotation
                var StrafeRotation = Mathf.Atan2(inputDirection.x, inputDirection.z) * Mathf.Rad2Deg + PlayerArmsController.MainCam.transform.eulerAngles.y;
                _targetRotation = StrafeRotation;
                var SmoothRotation = Mathf.Lerp(transform.eulerAngles.y, PlayerArmsController.MainCam.transform.eulerAngles.y, RotationSmoothTime);

                transform.rotation = Quaternion.Euler(0.0f, PlayerArmsController.MainCam.transform.eulerAngles.y, 0.0f);
